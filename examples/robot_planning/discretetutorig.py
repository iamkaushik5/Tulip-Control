...
sys.transitions.add_comb({'X1'}, {'X0', 'X1', 'X4', 'X2'})
sys.transitions.add_comb({'X2'}, {'X1', 'X2', 'X5'})
sys.transitions.add_comb({'X3'}, {'X0', 'X3', 'X4','X6'})
sys.transitions.add_comb({'X4'}, {'X3', 'X4', 'X1', 'X5', 'X7'})
sys.transitions.add_comb({'X5'}, {'X4', 'X2', 'X5', 'X8'})
sys.transitions.add_comb({'X6'}, {'X3', 'X6', 'X7'})
sys.transitions.add_comb({'X7'}, {'X6', 'X7', 'X8', 'X4'})
sys.transitions.add_comb({'X8'}, {'X8', 'X7', 'X5'})
# @system_dynamics_section_end@

# @system_labels_section@
# Add atomic propositions to the states
sys.atomic_propositions.add_from({'home', 'lot','obs1','obs4','obs7'})
sys.states.add('X0', ap={'home'})
sys.states.add('X8', ap={'lot'})
sys.states.add('X1', ap={'obs1'})
sys.states.add('X4', ap={'obs4'})
sys.states.add('X7', ap={'obs7'})
...
#
# @environ_section@
env_vars = {'park','obst1','obst4','obst7'}
# env_vars['obs'] = (1,7)
#env_vars['obs'] = (1, 7)
env_init = set()                # empty set
env_prog = {'!park'}
#env_safe = {'obs=1 -> X (obs=4 || obs=1)',
 #   'obs=4 -> X (obs=1 || obs=7 || obs=4)',
  #  'obs=7 -> X (obs=4 || obs=7)'}

env_safe={'obst1 -> X (obst4)',
	 'obst4 -> X (obst1 || obst7)',
	 'obst7 -> X (obst4)',
	'!(obst7 && obst4)','!(obst7 && obst1)', '!(obst1 && obst4)','(obst1 || obst4 || obst7)'}
...
sys_init = {'X0reach'}
sys_prog = {'home'}             # []<>home
sys_safe = {'(X (X0reach) <-> lot) || (X0reach && !park)','!(obst1 && obs1)','!(obst4 && obs4)', '!(obst7 && obs7)'}
sys_prog |= {'X0reach'}
...
#
# @plot_print@
if not ctrl.save('discretetut.png'):
    print(ctrl)
# @plot_print_end@
